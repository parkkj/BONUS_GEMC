...
map<string, G4Material*>  materialsWithIsotopes()
{
	...
	for(unsigned i=0; i<matTable->size(); ++i)
	{
		...
		if(thisMat->GetName() == "GasD2")
		{
			// Gasous D2 target for BONUS
			mats["GasD2"] = thisMat;
		}
	...
	// Deuterium gas
	mats["deuteriumGas"] = new G4Material("deuteriumGas", 0.000452*g/cm3, 1, kStateGas, 294.25*kelvin, 1*atmosphere);
	mats["deuteriumGas"]->AddElement(deuterium, 1);
	// Deuterium gas target for BONUS
	mats["GasD2"] = new G4Material("GasD2", 0.001229*g/cm3, 1, kStateGas, 293.*kelvin, 7.5*atmosphere);
	mats["GasD2"]->AddElement(deuterium, 1);
