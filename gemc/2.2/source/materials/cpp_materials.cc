...
// adding Gasious D2 material as BONUS target ... K.Park @ Oct.14, 2015
// adding He/DME gas mixture for BONUS12 ...   K.Park @ Oct.20, 2015
...
map<string, G4Material*> cpp_materials::initMaterials(runConditions rc, goptions opts)
{
	...
	// Mixture of Gas He/DME -----------------------------------------
	//	const double mDMEGasD_STP=1.97*mg/cm3;
	//	const double mHeGasD_STP=0.1786*mg/cm3;
	const double mDMEGasD_STP=1.97;
	const double mHeGasD_STP=0.1786;
	const double mMixGasP =1*atmosphere;
	const double mMixGasT = 293.*kelvin; // room temprature
	//const double mRatioHe2DME=82./18.;
	const double mRatioHe2DME=90./10.;
	double mMixHeGasD,mMixDMEGasD,mMixDMEHeD;

	mMixHeGasD=mHeGasD_STP*mMixGasP/atmosphere*273.15*kelvin/mMixGasT;
	mMixDMEGasD=mDMEGasD_STP*mMixGasP/atmosphere*273.15*kelvin/mMixGasT;
	mMixDMEHeD=(mMixHeGasD*mRatioHe2DME+mMixDMEGasD)/(mRatioHe2DME+1.0);
	a = 4.0026*g/mole;
	density = mMixHeGasD ;
	pressure= mMixGasP ;
	temperature = mMixGasT ;
	G4Material *mixHeGas = new G4Material("MixHeliumGas", z=2., a, density, kStateGas, temperature, pressure);

	//DMEGas((CH3-O-CH3) at 1 atm & room temprature, density=1.871 *mg/cm3;
	density = mMixDMEGasD ;
	G4Material *DMEGas = new G4Material("DMEGas", density, nel=3, kStateGas, temperature, pressure);
	DMEGas->AddMaterial(matman->FindOrBuildMaterial("G4_H"), 13.0*perCent);
	DMEGas->AddMaterial(matman->FindOrBuildMaterial("G4_C"), 52.2*perCent);
	DMEGas->AddMaterial(matman->FindOrBuildMaterial("G4_O"), 34.8*perCent);

	//BONUS Gas at 1 atm & room temprature, 82% Helium gas and 18% DME gas
	//BONUS Gas at 1 atm & room temprature, 90% Helium gas and 10% DME gas for BONUS12
	double WeightRatioDME=46.07/(4.0026*mRatioHe2DME+46.07);
	density = mMixDMEHeD;
	//	G4Material *bonusGas = new G4Material("bonusGas", density, nel=2, kStateGas, temperature=293.*kelvin, pressure=1.*atmosphere);
	G4Material *bonusGas = new G4Material("bonusGas", density*mg/cm3, nel=2, kStateGas, 293.*kelvin, 1.*atmosphere);
	bonusGas->AddMaterial(DMEGas, WeightRatioDME);
	bonusGas->AddMaterial(mixHeGas, 1.0-WeightRatioDME);
	//cout << "debugging :: density of MixHeGasD =" << mMixHeGasD << ", " << mMixDMEGasD << ", "<<  WeightRatioDME << endl;
	// end of Mixture gas material -----------------------------------------------------
	...
	//	G4Material *GasD2          = new G4Material("GasD2", z=1., a=  2.014*g/mole,   density =  1.229*mg/cm3,  kStateGas,  293.*kelvin, 7.5*atmosphere);
	//	G4Material *GasD2          = new G4Material("GasD2", z=1., a=  2.014*g/mole,   density =  1.229*mg/cm3,  kStateGas,  temperature=293.*kelvin, pressure=7.5*atmosphere);
	// 7.5 atm & 300K >> density= 0.12273*mg/cm3 
...
//	Scintillator->AddElement(H, 10);
	...
	G4Material* He4_1atm = new G4Material( "He4_1atm",  density = 1.*273.15/293.0*0.1786*mg/cm3, nel=1 );
	...
	//	MMats["GasD2"]            = GasD2;      // Gasous D2 target (7.5atm, room-temp:293*kelvin)
	MMats["bonusGas"]       = bonusGas; // mixture gas He/DME for BONUS12
	MMats["LHe"]            = LHe;
