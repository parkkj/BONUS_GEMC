...
//________________________________________________________________________________________________
 ...
 // The same but with random error following a gaussian distribution added
   ...
   double PBeamMC = 70.; // input of spectator proton momentum 
   ...
   for (int iEvt=0; iEvt<NEvts; iEvt++) {
     if(iran){
       double PBeamMCx= PBeamMC*ran3.Gaus(0.7,0.01);
       double PBeamMCy= PBeamMC*ran3.Gaus(0.7,0.01);
       double PBeamMCz= PBeamMC*ran3.Gaus(0.001,0.01);
     }
     else{
       double PBeamMCx= PBeamMC/sqrt(2);
       double PBeamMCy= PBeamMC/sqrt(2);
       double PBeamMCz= PBeamMC*0.0001;    
     }
   ...
   cout << "X=" << PX_gen  << " Y=" << PY_gen  << " Z=" << PZ_gen  << endl;
...
// Simulationtion parameters (Are these precision of pseudo data??)
   double sigX = 0.3e-3;
   double sigY = 0.3e-3;
   double sigZ = 0.5e-3;
...
// As the Kalman filter needs to be initialized we use the global Helix Fit for this. It will also be used afterwards for comparison
   ...
   OUT << setiosflags(ios::left) << setiosflags(ios::fixed) << endl;
