...
//________________________________________________________________________________________________
   ...
   const double MeVcToSi = 5.34428595e-22; // constant to change MeV/c in standard units
...
//   double sigMomloss = 4e-5 * MeVcToSi; // Eloss per step (added by hand nothing physical here)
   double sigMomloss = 4e-5* MeVcToSi;
 ...
 // The same but with random error following a gaussian distribution added
   ...
   double PX_gen = MeVcToSi*10.; // 10 MeV/c
   double PY_gen = MeVcToSi*20.; // 20 Mev/c
   double PZ_gen = MeVcToSi*40; // 40 MeV/c
...
//________________________________________________________________________________________________
   ...
   TCanvas *c0 = new TCanvas("c0","View of the Kalman filter effect",800,600);
   TCanvas *cX = new TCanvas("cX","View of the Kalman filter for X",800,600);
   TCanvas *cY = new TCanvas("cY","View of the Kalman filter for Y",800,600);
   TCanvas *cZ = new TCanvas("cZ","View of the Kalman filter for Z",800,600);
   TCanvas *cXY = new TCanvas("cxy","View of the Kalman filter for xy",800,600);
   TCanvas *cdR = new TCanvas("cdR","R growth",800,600);

   TCanvas *cPX = new TCanvas("cPX","View of the Kalman filter for PX",800,600);
   TCanvas *cPY = new TCanvas("cPY","View of the Kalman filter for PY",800,600);
   TCanvas *cPZ = new TCanvas("cPZ","View of the Kalman filter for PZ",800,600);
   TCanvas *cPT = new TCanvas("cPT","View of the Kalman filter for PT",800,600);
   ...
   TH1D *h_Rgrowth = new TH1D("h_Rgrowth","R growth",100,7e-6.,1e-5);
...
// As the Kalman filter needs to be initialized we use the global Helix Fit for this. It will also be used afterwards for comparison
   ...
   cout << "Phi resolution: " << tmpPhi*TMath::Pi()/180. - phi_gen << " rad." << endl;
   cout << "Theta resolution: " << tmpTheta*TMath::Pi()/180. - theta_gen << " rad." << endl;
   cout << "Z resolution: " << tmpZ - Z_gen << " mm." << endl;
   cout << "******************************************************************************" << endl;
   cout << "  " << endl;
...
//if(hit>NHit-50) cout << hit << "  " << TMath::Sqrt(MeasX[hit]*MeasX[hit]+MeasY[hit]*MeasY[hit]) << "  " << TMath::Sqrt(Xkm(0,0)*Xkm(0,0)+Xkm(1,0)*Xkm(1,0)) << "  " << dR << endl;
   ...
   cout << "Phi resolution: " << phi_kal - phi_gen << " rad." << endl;
   cout << "Theta resolution: " << theta_kal - theta_gen << " rad." << endl;
   cout << "Z resolution: " << Xk(2,0) - Z_gen << " mm." << endl;
   cout << "******************************************************************************" << endl;
...
cout << "Particle path length: " << DistTot << " m in " << (double) (NHit*dt) << " s, i.e. a speed of " << DistTot/ (double) (NHit*dt) << " m/s." << endl;
