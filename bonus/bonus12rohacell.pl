...
sub make_sensitive_he1
{
	...
	{
		$phistart =  90 + (180 - $pspan)/2.0;
		$detector{"name"} = "sensitive_he1_left";
	}
	
	if($which == 2)
	{
		$phistart = 270 + (180 - $pspan)/2.0;
		$detector{"name"} = "sensitive_he1_right";
	}
	
	$detector{"mother"}      = "bonus";
	$detector{"description"} = "Sensitive He vol1";
...
sub make_sensitive_he2
{
	...
	{
		$phistart =  90 + (180 - $pspan)/2.0;
		$detector{"name"} = "sensitive_he2_left";
	}
	
	if($which == 2)
	{
		$phistart = 270 + (180 - $pspan)/2.0;
		$detector{"name"} = "sensitive_he2_right";
	}
	
	$detector{"mother"}      = "bonus";
	$detector{"description"} = "Sensitive He Vol2";
	$detector{"color"}       = "ff88994";
	$detector{"type"}        = "Tube";
	$detector{"dimensions"}  = "$rmin*mm $rmax*mm $z_half*mm $phistart*deg $pspan*deg";
#	$detector{"material"}    = "G4_He";
...
# last electronics readout layer
...
make_sensitive_he1(1);
make_sensitive_he1(2);

make_sensitive_he2(1);
make_sensitive_he2(2);
